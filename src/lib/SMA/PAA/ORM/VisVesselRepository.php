...
class VisVesselRepository extends OrmRepository
{
    ...
    public function saveValidImo(OrmModel $model, bool $skipCrudLog = false)
    {
        ...
        # Keep vessel repository in sync with vis vessel repository
        ...
        # Check if service id already exists in database
        $visVesselModel = $this->first(["service_id" => $model->service_id]);

        # If service id is already stored, then just update other information
        if (isset($visVesselModel)) {
            ...
            return parent::save($visVesselModel);
        }
    ...
    public function saveWithoutImo(OrmModel $model, bool $skipCrudLog = false)
    {
        # Check if service id already exists in database
        # Database will not have entries without IMO
        $visVesselModel = $this->first(["service_id" => $model->service_id]);
        if (isset($visVesselModel)) {
            $model->imo = $visVesselModel->imo;
            return $this->saveValidImo($model, $skipCrudLog);
        }
    ...
    public function getWithServiceId(string $serviceId): ?OrmModel
    {
        return $this->first(["service_id" => $serviceId]);
    }

    public function getVesselNameWithServiceId(string $serviceId): string
    {
        $res = "";
